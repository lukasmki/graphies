...
dataset = CSVGraphiesDataset(root / "fourcolor.csv", "graphies", tokenizer)


# trn, tst = random_split(dataset, [0.9, 0.1])
# torch.save({"trn_indices": trn.indices, "tst_indices": tst.indices}, chk / "split.pt")
indices = torch.load(chk / "split.pt")
trn = Subset(dataset, indices=indices["trn_indices"])
tst = Subset(dataset, indices=indices["tst_indices"])
...
trainer = GraphiesTrainer.from_checkpoint(
