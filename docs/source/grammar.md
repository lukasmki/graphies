...
Nodes can be represented in the form `[{edge_symbol}{node_symbol}{modifier_symbols}]`.
A sequence of these nodes instances is a sufficient representation of a chain of nodes with varying itnernalstate and edge types. For a trivial example, hexane would look like this: `[C][C][C][C][C][C]`.
...
Starting with branches, we could introduce a SMILES-like syntax for branching by enclosing some part of the token sequence with parentheses to indicate a new chain to attach to the previous node. However, this would break our ability to parse sequences with unterminated branches. More importantly, it introduces a severe fragility to the string representation as a single token modification would drastically alter the structure. The solution developed by SELFIES is to pair the branch token with a set of numerical tokens that indicate the length of the branch.
